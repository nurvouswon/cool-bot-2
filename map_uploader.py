...
# Your Snowflake connection details (adjust/stash securely)
...
# === Helper to convert nested park_hand_hr_rate_map to flat list ===
park_hand_hr_rows = []
for park, hand_dict in park_hand_hr_rate_map.items():
    for hand, hr_rate in hand_dict.items():
        park_hand_hr_rows.append({'park': park, 'hand': hand, 'hr_rate': hr_rate})
...
for name, df in [
    ...
    create_and_upload(name, df)

cur.close()
conn.close()
print("Done uploading all mapping tables!")
