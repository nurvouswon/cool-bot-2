...
with tab1:
    ...
    if fetch_btn:
        ...
        # --- Identify HRs BEFORE filter for debugging
        if 'hr_outcome' not in df.columns and 'events' in df.columns:
            df['hr_outcome'] = df['events'].astype(str).str.lower().str.replace(' ', '').isin(['homerun', 'home_run']).astype(int)
        st.markdown("hr_outcome counts BEFORE filter:")
        ...
        # --- Identify HRs again in filtered data
        if 'hr_outcome' not in df.columns and 'events' in df.columns:
            df['hr_outcome'] = df['events'].astype(str).str.lower().str.replace(' ', '').isin(['homerun', 'home_run']).astype(int)
        st.markdown("hr_outcome counts AFTER filter:")
        st.dataframe(df['hr_outcome'].value_counts().rename("count").to_frame())
        ...
        # Check for enough events *after* filtering
        outcome_counts = df['hr_outcome'].value_counts()
        if (1 in outcome_counts.index and 0 in outcome_counts.index and outcome_counts[1] > 1 and outcome_counts[0] > 1):
            ...
            # Fit only if both classes still present
