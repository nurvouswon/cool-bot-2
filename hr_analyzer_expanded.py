...
# ------------------- Park Mapping (robust for naming variants) ------------------- #
team_code_to_park = {
...
# Fix for "tmobile_park" spelling in Statcast
park_aliases = {'tmobile_park': 't-mobile_park', 'loan_depot_park': 'loan_depot_park', 'loandepot_park': 'loan_depot_park'}
...
park_hr_rate_map = {
    ...
    'target_field': 0.95  # Added missing
}
...
park_altitude_map = {
    ...
    'nationals_park': 25, 'american_family_field': 633, 'sutter_health_park': 20, 'target_field': 850
}
...
def safe_add_rolling(df, id_col, stat_cols, windows, prefix):
    found = []
    for stat in stat_cols:
        if stat not in df.columns:
            st.warning(f"Missing advanced stat: {stat}")
            continue
        for w in windows:
...
if run_query:
    ...
    # --- Filter only target batted ball events
    ...
    def get_park(ht):
        code = str(ht).upper()
        park = team_code_to_park.get(code, code.lower())
        park = park_aliases.get(park, park)
        return park

    df['park'] = df['home_team'].apply(get_park)
    ...
    # --------------- Advanced rolling and context features --------------- #
    ROLL_WINDOWS = [3, 5, 7, 14]
    # Add all known advanced Statcast rolling stats you want, will skip missing.
    batter_stats = ['launch_speed', 'launch_angle', 'hit_distance_sc', 'woba_value', 'iso_value', 'xba', 'xwoba', 'xslg']
    pitcher_stats = ['launch_speed', 'launch_angle', 'hit_distance_sc', 'woba_value', 'iso_value', 'xba', 'xwoba', 'xslg']

    df, batter_roll_feats = safe_add_rolling(df, 'batter_id', batter_stats, ROLL_WINDOWS, 'B')
    df, pitcher_roll_feats = safe_add_rolling(df, 'pitcher_id', pitcher_stats, ROLL_WINDOWS, 'P')

    # --- Pitch mix % by batter and pitcher (robust)
    pitch_types = sorted(df['pitch_type'].dropna().unique())
    pitchmix_windows = ROLL_WINDOWS
    for pt in pitch_types:
        for w in pitchmix_windows:
            b_col = f"B_pitchmix_{pt}_{w}"
            p_col = f"P_pitchmix_{pt}_{w}"
            # For batter
            try:
                df[b_col] = (
                    df.groupby('batter_id')['pitch_type']
                    .transform(lambda x: x.shift(1).rolling(w, min_periods=1)
                               .apply(lambda y: (y == pt).mean() if len(y) else np.nan))
                )
            except Exception:
                df[b_col] = np.nan
            # For pitcher
    ...
    # --- Context features: handedness, pitch, platoon, etc
    ...
    export_cols = [
        ...
        'stand', 'p_throws', 'park', 'park_hr_rate', 'park_altitude',
        'temp', 'wind_mph', 'wind_dir', 'humidity', 'condition',
        'handed_matchup', 'primary_pitch', 'platoon', 'is_day',
        *batter_roll_feats, *pitcher_roll_feats,
        *[f"B_pitchmix_{pt}_{w}" for pt in pitch_types for w in pitchmix_windows],
        *[f"P_pitchmix_{pt}_{w}" for pt in pitch_types for w in pitchmix_windows],
        'pull_air', 'flyball', 'line_drive', 'groundball', 'pull_side',
        'hr_outcome'
    ]
    export_cols = [c for c in export_cols if c in df.columns]  # Robust filtering
    ...
    player_cols = ['batter_id', 'batter'] + [c for c in batter_roll_feats if c in event_df.columns]
    player_df = (
    ...
    logit_features = [
        c for c in event_df.columns if (
            any(x in c for x in [
                'launch_speed', 'launch_angle', 'hit_distance', 'woba_value', 'iso_value', 'xba', 'xwoba', 'xslg',
                'pitchmix_', 'park_hr_rate', 'platoon', 'temp', 'wind_mph', 'humidity', 'pull_air', 'flyball',
                'line_drive', 'groundball', 'pull_side'
            ])
        )
    ]
    ...
    if len(model_df) > 0:
        ...
        st.warning("Not enough data to fit logistic regression.")

    st.success("Full analysis done! All rolling/context features, pitch mix, and advanced stats (when present) included.")
