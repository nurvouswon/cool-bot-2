...
with tab1:
    ...
    if fetch_btn:
        ...
        df_out = df.copy()
        # Some columns may not exist in data, that's fine for output
        ...
        st.download_button("⬇️ Download Event-Level CSV (aligned for today CSV)", data=df_out.to_csv(index=False), file_name="event_level_hr_features_aligned.csv", key="download_aligned_event_level")
