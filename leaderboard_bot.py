...
# Park to city mapping (expand for all ballparks)
PARK_CITY = {
    "Yankee Stadium": "Bronx", "Fenway Park": "Boston", "Tropicana Field": "St. Petersburg",
    "Camden Yards": "Baltimore", "Oriole Park at Camden Yards": "Baltimore",
    "Rogers Centre": "Toronto", "Comerica Park": "Detroit", "Progressive Field": "Cleveland",
    "Target Field": "Minneapolis", "Kauffman Stadium": "Kansas City", "Guaranteed Rate Field": "Chicago",
    "Angel Stadium": "Anaheim", "Minute Maid Park": "Houston", "Oakland Coliseum": "Oakland",
    "T-Mobile Park": "Seattle", "Globe Life Field": "Arlington", "Dodger Stadium": "Los Angeles",
    "Chase Field": "Phoenix", "Coors Field": "Denver", "Oracle Park": "San Francisco",
    "Wrigley Field": "Chicago", "Great American Ball Park": "Cincinnati", "American Family Field": "Milwaukee",
    "PNC Park": "Pittsburgh", "Busch Stadium": "St. Louis", "Truist Park": "Atlanta",
    "LoanDepot Park": "Miami", "Citi Field": "Queens", "Nationals Park": "Washington",
    "Petco Park": "San Diego", "Citizens Bank Park": "Philadelphia", "Globe Life Park in Arlington": "Arlington",
    "Sahlen Field": "Buffalo", "Chukchansi Park": "Fresno"
}

PARK_FACTORS = {
    "Yankee Stadium": 1.19, "Fenway Park": 0.97, "Coors Field": 1.30, "Globe Life Field": 1.00, "Tropicana Field": 0.85,
    "Chase Field": 1.06, "Oracle Park": 0.82, "T-Mobile Park": 0.86, "Dodger Stadium": 1.10, "Wrigley Field": 1.12,
    "Great American Ball Park": 1.26, "American Family Field": 1.17, "Progressive Field": 1.01,
    "Busch Stadium": 0.87, "Petco Park": 0.85, "Guaranteed Rate Field": 1.18, "LoanDepot Park": 0.86,
    "Citi Field": 1.05, "Minute Maid Park": 1.06, "Camden Yards": 1.13, "Rogers Centre": 1.10,
    "Comerica Park": 0.96, "Kauffman Stadium": 0.98, "Angel Stadium": 1.05, "Oakland Coliseum": 0.82,
    "Nationals Park": 1.05, "Citizens Bank Park": 1.19, "Target Field": 1.04, "Truist Park": 1.06,
    "PNC Park": 0.87, "Globe Life Park in Arlington": 1.00, "Sahlen Field": 0.95, "Chukchansi Park": 0.95
}
...
def fetch_schedule_df(date_str):
    ...
    try:
        ...
        for g in games:
            game_pk = g["gamePk"]
            venue = g["venue"]["name"]
            city = PARK_CITY.get(venue, g["venue"].get("location", "TBD"))
...
def feature_prepare(df):
    ...
    for col in ['temp_f','wind_mph','humidity','park_factor','launch_speed','launch_angle',
                ...
                'sweet_spot_percent','pull_percent','oppo_percent','gb_percent','pitch_type_fastball',
                'pitch_type_offspeed','whiff_rate','contact_rate','k_rate','bb_rate','xwoba','xslg']:
...
st.title("MLB HR Backtest + ML + Leaderboard Bot (Cool Bot 2, Batted Ball, Weather, Handedness, All Advanced Stats)")
...
if mode.startswith("Build"):
    ...
    if st.button("Build CSV"):
        ...
        sweet_spot, pull, oppo, gb, fastball, offspeed, whiff, contact, k, bb, xwoba, xslg = [],[],[],[],[],[],[],[],[],[],[],[]
        ...
        for idx, row in df.iterrows():
            ...
            sweet_spot.append(row.get('sweet_spot_percent', np.nan))
            pull.append(row.get('pull_percent', np.nan))
            oppo.append(row.get('oppo_percent', np.nan))
            gb.append(row.get('gb_percent', np.nan))
            fastball.append(row.get('pitch_type_fastball', np.nan))
            offspeed.append(row.get('pitch_type_offspeed', np.nan))
            whiff.append(row.get('whiff_rate', np.nan))
            contact.append(row.get('contact_rate', np.nan))
            k.append(row.get('k_rate', np.nan))
            bb.append(row.get('bb_rate', np.nan))
            xwoba.append(row.get('xwoba', np.nan))
            xslg.append(row.get('xslg', np.nan))
            city_val = row['city'] if pd.notnull(row['city']) else PARK_CITY.get(row['venue'], "TBD")
        ...
        df['sweet_spot_percent'] = sweet_spot
        df['pull_percent'] = pull
        df['oppo_percent'] = oppo
        df['gb_percent'] = gb
        df['pitch_type_fastball'] = fastball
        df['pitch_type_offspeed'] = offspeed
        df['whiff_rate'] = whiff
        df['contact_rate'] = contact
        df['k_rate'] = k
        df['bb_rate'] = bb
        df['xwoba'] = xwoba
        df['xslg'] = xslg
        ...
        cols = [
            ...
            'sweet_spot_percent','pull_percent','oppo_percent','gb_percent','pitch_type_fastball','pitch_type_offspeed',
            'whiff_rate','contact_rate','k_rate','bb_rate','xwoba','xslg',
...
# ===== TRAIN MODEL =====
    ...
    ml_model_type = st.radio("Select model", ["RandomForest", "LightGBM"] if lgb else ["RandomForest"])
    if uploaded:
        ...
        features = [
            ...
            'sweet_spot_percent', 'pull_percent', 'oppo_percent', 'gb_percent',
            'pitch_type_fastball', 'pitch_type_offspeed', 'whiff_rate', 'contact_rate',
            'k_rate', 'bb_rate', 'xwoba', 'xslg'
        ]
        ...
        if ml_model_type == "RandomForest":
            ...
            model = lgb.train(
                {'objective': 'binary', 'metric': 'auc', 'verbosity': -1, 'num_leaves': 16, 'seed': 42},
                lgb_train, num_boost_round=100, valid_sets=[lgb_eval], verbose_eval=False
            )
        ...
        with open("mlb_hr_model.pkl", "wb") as f:
            pickle.dump((model, ml_model_type), f)
        st.success(f"Model trained and saved as mlb_hr_model.pkl ({ml_model_type})")
    ...
    if uploaded:
        try:
            with open("mlb_hr_model.pkl", "rb") as f:
                model, ml_model_type = pickle.load(f)
        except Exception as e:
        ...
        feats = [f for f in features if f in df_pred.columns]
        X_new = df_pred[feats]
        if ml_model_type == "RandomForest":
...
# ===== LEADERBOARD =====
    ...
    if uploaded:
        ...
        if 'HR_Prob' not in df.columns:
            ...
            leaderboard_cols = [c for c in ['batter_name','pitcher_name','venue','HR_Prob','batter_hand','pitcher_hand'] if c in df.columns]
            st.write(df.sort_values("HR_Prob", ascending=False).head(25)[leaderboard_cols])
